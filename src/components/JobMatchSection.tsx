    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex items-center space-x-1.5">
                <span className="text-[11px] text-[#78716C] hidden sm:inline">Load Sample:</span>
                {SAMPLE_JDS.map((jd, idx) => (
                  <button
                    ...
                    className="text-[11px] font-semibold text-[#57534E] hover:text-emerald-700 px-2.5 py-1 rounded-lg bg-[#FAF7F2] border border-[#E8E4DC] hover:border-emerald-300 transition-colors cursor-pointer"
                  >
            ...
            <textarea
              ...
              className="w-full p-3 bg-[#FAF7F2] border border-[#E8E4DC] rounded-xl text-xs text-[#1C1917] placeholder-stone-400 focus:outline-none focus:border-emerald-600 font-mono leading-relaxed"
            />
          </div>
      ...
      {matchResult && (
        <div className="space-y-5 animate-fade-in">
          ...
          <div className="bg-white border border-[#E8E4DC] rounded-2xl p-6 transition-colors space-y-4 shadow-xs">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E8E4DC] pb-4">
              <div>
                <div className="flex items-center space-x-2">
                  <span className="text-xs font-bold text-[#78716C] uppercase">Match Result</span>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              {/* Matched Keywords */}
              <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#E8E4DC] space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {matchResult.matchingKeywords.map((kw, i) => (
                    <span key={i} className="px-2 py-0.5 bg-white text-emerald-800 border border-emerald-200 rounded-md text-xs font-medium shadow-2xs">
                      {kw}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#E8E4DC] space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {matchResult.missingKeywords.map((kw, i) => (
                    <span key={i} className="px-2 py-0.5 bg-white text-rose-800 border border-rose-200 rounded-md text-xs font-medium shadow-2xs">
                      {kw}
                    </span>
                  ))}
                </div>
          ...
          <div className="bg-white border border-[#E8E4DC] rounded-2xl p-6 transition-colors space-y-4 shadow-xs">
            ...
            <div className="space-y-3">
              {matchResult.tailoredBullets.map((bullet, idx) => (
                <div key={idx} className="bg-[#FAF7F2] p-4 rounded-xl border border-[#E8E4DC] space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-white hover:bg-emerald-50 text-[#57534E] border border-[#E8E4DC] rounded-lg text-xs font-semibold flex items-center space-x-1 shrink-0 transition-colors cursor-pointer"
                    >
                      {copiedId === `tb_${idx}` ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3 text-emerald-700" />}
                      <span>{copiedId === `tb_${idx}` ? 'Copied' : 'Copy'}</span>
                    </button>
                  </div>

                  <div className="flex items-center space-x-2 text-[11px] text-[#78716C]">
                    <span className="font-bold text-emerald-700">Keywords Weaved:</span>
