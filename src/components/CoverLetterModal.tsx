...
export const CoverLetterModal: React.FC<CoverLetterModalProps> = ({ analysis, token }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
                ...
                <button
                  ...
                  className={`p-2.5 rounded-lg border text-left transition-colors cursor-pointer ${
                    selectedTone === t.id
                      ...
                      : 'bg-slate-50 dark:bg-[#161f33] border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-slate-400'
                  }`}
                >
                  <p className="text-xs font-bold leading-tight">{t.label}</p>
                  <p className={`text-[10px] mt-0.5 leading-snug ${selectedTone === t.id ? 'text-slate-300 dark:text-slate-700' : 'text-slate-500 dark:text-slate-400'}`}>
                    {t.desc}
                  </p>
                </button>
              ))}
      ...
      {coverLetter && (
        <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-xl p-5 sm:p-6 space-y-4 animate-fade-in transition-colors">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 dark:bg-slate-100 dark:hover:bg-white text-white dark:text-slate-900 rounded-lg text-xs font-bold flex items-center space-x-1.5 transition-colors cursor-pointer"
              >
          ...
          <div className="bg-slate-50 dark:bg-[#161f33] p-5 rounded-lg border border-slate-200 dark:border-slate-700 font-sans text-xs sm:text-sm text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">
            {coverLetter}
          </div>
        </div>
      )}
