...
export const CoverLetterModal: React.FC<CoverLetterModalProps> = ({ analysis, token }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
        ...
        <form onSubmit={handleGenerate} className="mt-5 space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <Building className="w-4 h-4 text-[#78716C] absolute left-3.5 top-3" />
                <input
                  type="text"
                  required
                  value={companyName}
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border text-left transition-colors cursor-pointer ${
                    selectedTone === t.id
                      ...
                      : 'bg-[#FAF7F2] border-[#E8E4DC] text-[#57534E] hover:border-emerald-400'
                  }`}
                >
                  <p className="text-xs font-bold leading-tight">{t.label}</p>
                  <p className={`text-[10px] mt-0.5 leading-snug ${selectedTone === t.id ? 'text-emerald-100' : 'text-[#78716C]'}`}>
                    {t.desc}
                  </p>
                </button>
              ))}
      ...
      {coverLetter && (
        <div className="bg-white border border-[#E8E4DC] rounded-2xl p-6 space-y-4 animate-fade-in transition-colors shadow-xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E8E4DC] pb-3">
            ...
            <div className="flex items-center space-x-2">
              <button
                onClick={handleCopy}
                className="px-3 py-1.5 bg-[#FAF7F2] hover:bg-emerald-50 text-[#57534E] rounded-lg text-xs font-semibold flex items-center space-x-1.5 transition-colors cursor-pointer border border-[#E8E4DC]"
              >
          ...
          <div className="bg-[#FAF7F2] p-5 rounded-xl border border-[#E8E4DC] font-sans text-xs sm:text-sm text-[#1C1917] leading-relaxed whitespace-pre-wrap">
            {coverLetter}
          </div>
        </div>
      )}
